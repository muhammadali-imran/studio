...
export default function AttendanceListPage() {
  ...
  const { data } = useApi(`/studio/courses/${id}/sessions/`)
  const { mutate: del } = useMutation(null, 'delete')
  const [deleteTarget, setDeleteTarget] = useState(null)
  const sessions = data ?? mockSessions

  const avgAttendance = sessions.length > 0
    ? Math.round(sessions.reduce((s, sess) => s + (sess.present / sess.total) * 100, 0) / sessions.length)
    : 0

  const handleDelete = async () => {
    try { await del(null, `/studio/sessions/${deleteTarget}/`); toast.success('Session deleted.') }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-3 gap-4">
        ...
        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-5 text-center">
          <p className="text-2xl font-bold text-slate-800">{sessions[0]?.total ?? 0}</p>
