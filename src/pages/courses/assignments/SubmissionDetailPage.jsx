...
export default function SubmissionDetailPage() {
  ...
  const { data: submission, loading } = useApi(`/studio/submissions/${sid}/`)
  const { mutate: grade } = useMutation(`/studio/submissions/${sid}/grade/`, 'post')

  const s = submission || mockSubmission
    ...
    <div className="space-y-6">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <h2 className="text-2xl font-bold text-slate-800 mt-1">{s.student_name}</h2>
          <p className="text-slate-500 text-sm mt-0.5">{s.student_email}</p>
        </div>
        <div className="flex gap-2">
          {s.late && <Badge variant="red">Late</Badge>}
          {s.grade !== null && <Badge variant="green">Graded: {s.grade}/100</Badge>}
        </div>
      </div>

      <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-6">
        <SubmissionReview submission={s} maxGrade={100} onGrade={handleGrade} />
      </div>
    </div>
  )
}
